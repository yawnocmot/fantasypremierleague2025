...
fpl_gw <- read_csv("processed_data/fpl_gw.csv")
fpl_fixtures <- read_csv("processed_data/fpl_fixtures.csv")
fixtures_poisson_format <- read_csv("processed_data/fixtures_poisson_format.csv")
```
...
# Look at what proportion are scored by each player - cap at 0.4 because it is rare that a player will sustain over 40% of their team's attacking output even if it happens in the short term.

player_data <- player_data |>
  left_join(goals_per_team) |>
  mutate(prop_goals = pmin(goals_weighted/team_goals,0.4),
         prop_assists = pmin(assists_weighted/team_assists,0.4))
...
# Want goals against for the season
...
# Also what happens next week for selecting who to play

next_week <- team_scores_per_week |>
  arrange(round_number)|>
  head(1)|>
  pull(round_number)

stats_next_week <- team_scores_per_week |> 
  filter(round_number == next_week)|>
  select(short_name = attacker_short_name, 
         next_goals = goals, 
         next_clean_sheet = prob_clean_sheet,
         next_concede_2 = prob_concede_2,
         next_concede_4 = prob_concede_4)
...
player_data <- player_data |>
  ...
  mutate(
    ...
    player_goals_next = next_goals*prop_goals,
    player_assists_next = next_goals*prop_assists,
    player_clean_sheets_next = minutes_60_weighted * next_clean_sheet,
    player_concede_2_next = minutes_60_weighted * next_concede_2,
    player_concede_4_next = minutes_60_weighted * next_concede_4
  ) |>
  ...
  mutate(
    ...
    predicted_next_week_points = case_when(
      element_type == 1 ~ minutes_any_weighted + minutes_60_weighted
                          + 4*player_clean_sheets_next
                          + 10*player_goals_next + 3*player_assists_next
                          - player_concede_2_next - player_concede_4_next, 
      element_type == 2 ~ minutes_any_weighted + minutes_60_weighted
                          + 4*player_clean_sheets_next
                          + 6*player_goals_next + 3*player_assists_next
                          - player_concede_2_next - player_concede_4_next,
      element_type == 3 ~ minutes_any_weighted + minutes_60_weighted 
                          + 1*player_clean_sheets_next
                          + 5*player_goals_next + 3*player_assists_next,
      element_type == 4 ~ minutes_any_weighted + minutes_60_weighted  
                          + 4*player_goals_next + 3*player_assists_next
    ) * if_else(is.na(play_chance),1,play_chance/100)
  )
...
-   Export data as CSV for team picker:
