...
-   Only run this page when selecting a team - initially or as a wild-card. Put week-by-week changes into page 06.
...
worst_seed <- seeds$seeds[[50]]$selection |> left_join(ranked_players|> select(id, web_name), by = join_by(.id == id))|> print(n=16)
...
-   Pairwise swaps to improve from the seed
...
#Best seed still the same for this. 
...
# ---- Phase A: 24 seeds × 600 steps (≈ 3–5 mins on most laptops) ----
set.seed(125)
...
# Team picked - don't have in place captaincy so assume Salah. Go back and work in 'next game' codes - and see the effect of adjusting parameters around future games.

```

-   Picking a captain within the team - likely score this week.

-   Once the game has started - incorporate live data, substitutions.
